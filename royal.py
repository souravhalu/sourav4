...
bot = telebot.TeleBot('7671173605:AAGamlSQseBMOlvwgD9zrHwn9IGNrAs6JLA')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 180:
                response = "You Are On Cooldown. Please Wait 4min Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 800:
                ...
                full_command = f"./royal {target} {port} {time}"
